...
async function attach(){
    ...
    let attachURL = await getRemoteLocalURL("attach.html");
...
async function detachWindow(id){
    let window = data.linkedWindows[id];
    let detachURL = await getRemoteLocalURL("detach.html");
    API.spawnOverlay({
        posX: window.transform.posX,
        posY: window.transform.posY,
        posZ: window.transform.posZ,
...
async function getUtilityOverlay(transform){
    if(utility===null){
        let utilityURL = await getRemoteLocalURL("utility.html");
